...
begin
	loot_gold_prices = rand(1:3, 1000)
	loot_values = 5*loot_gold_prices .+ rand(1:3, 1000)
	attacking_probs = 0.3 .+ rand(0.0:0.01:0.4, 1000)
	adventurers = [Adventurer(0, 0, 100, 5, AttackingPolicy(attacking_probs[i],  
		           1 - attacking_probs[i]),
				   AdventurerSelling(loot_values[i], loot_gold_prices[i]), 			  
				   1, WEAPONS[1]) for i in range(1,1000)]
end

# ╔═╡ 4c2d22c4-3161-4d99-ab76-f52621bfc251
begin
	craftsmen = [Craftsman(0, 1000, nothing, PricingPolicy(rand(100:125), 
		         rand(250:300)), 1, 0) for i in range(1,1000)] 
end

# ╔═╡ 99abcb33-a93a-406e-a44f-96e6ef17f98c
begin
	loot_purchasing_data = LootPurchasingData(Int64[], Int64[])
	weapon_purchasing_data = WeaponPurchasingData(Weapon[], Int64[])
	play!(adventurers, craftsmen, 50, 10000, loot_purchasing_data, 			
		  weapon_purchasing_data)
end
...
# ╔═╡ fe6ea856-c452-4aed-8702-d9acf12bec39
loot_unit_price = loot_purchasing_data.loot_prices./
				  loot_purchasing_data.loot_packages

# ╔═╡ f5eded03-b5c6-4d6c-8322-7013461eaaad
sliced_loot_unit_price = loot_unit_price[1:end-length(loot_unit_price)%10000]

# ╔═╡ 05d8cb98-55f6-408a-abee-5d8b1bc2c7eb
mean_loot_unit_price = mean.(
	[sliced_loot_unit_price[10000*(i-1)+1:10000*i] for
	i in range(1, div(length(sliced_loot_unit_price), 10000))]
)

# ╔═╡ 668b4578-2df5-4aa7-bf56-0533e2de562a
plot(mean_loot_unit_price, label="mean loot unit price",
	 title="Loot unit price per time")

# ╔═╡ f7d639a1-8b7f-4b1a-aa8f-de65dc8b1fa4
begin
    warrior_weapon_price = weapon_purchasing_data.prices[
		   typeof.(weapon_purchasing_data.weapons) .== WarriorWeapon]
	legendary_weapon_price = weapon_purchasing_data.prices[
		   typeof.(weapon_purchasing_data.weapons) .== LegendaryWeapon]
	ultimate_weapon_price = weapon_purchasing_data.prices[
		   typeof.(weapon_purchasing_data.weapons) .== UltimateWeapon]
end

# ╔═╡ 202f7620-aa3a-4539-b3f9-5d82a8505ece
sliced_warrior_weapon_price = warrior_weapon_price[1:end-
      length(warrior_weapon_price)%10]

# ╔═╡ eddc4866-448c-4cd2-881a-123ce16d7e0c
mean_warrior_weapon_price = mean.(
	[warrior_weapon_price[10*(i-1)+1:10*i] for
	i in range(1, div(length(warrior_weapon_price), 10))]
)

# ╔═╡ 8ba830ca-b5b3-480b-bf9e-ed3085d3d2ce
plot(mean_warrior_weapon_price, label="mean warrior weapon price")

# ╔═╡ d5e5d58d-d4e8-45b6-a0b7-ce356a33d7f6
sliced_legendary_weapon_price = legendary_weapon_price[1:end-
      length(legendary_weapon_price)%10]

# ╔═╡ 8adde09c-2db7-42ba-8d04-5d2506111300
mean_legendary_weapon_price = mean.(
	[legendary_weapon_price[10*(i-1)+1:10*i] for
	i in range(1, div(length(legendary_weapon_price), 10))]
)

# ╔═╡ 0b0c02c2-9694-4ebe-b8fd-df120eb7c3c5
plot(mean_legendary_weapon_price, label="mean legendary weapon price")

# ╔═╡ 21096138-10fe-4b59-b403-c531add0c53a
plot(ultimate_weapon_price, label="ultimate weapon price")
